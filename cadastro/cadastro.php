<?php
	...
	if($validate){
		...
		$s = "SELECT indice FROM ".PFIX."user_login WHERE email='$email' ";
		$r = $Qry->query($s);
		$l = $Qry->rows($r);
		if($l){
			$valEmail = false;
		...
		if(!$valCPF){
			...
			$s = "INSERT INTO ".PFIX."user_login (cpf, email, senha, time) VALUES ('$cpf','$email','$senha','$time')";
			if($r = $Qry->query($s)){
				//recuperando o indice e gerando o UID
				$s = "SELECT indice FROM ".PFIX."user_login WHERE cpf = '$cpf' AND email = '$email'";
				$r = $Qry->query($s);
				$l = $Qry->rows($r);
				if($l){
					$d = $Qry->arr($r);
